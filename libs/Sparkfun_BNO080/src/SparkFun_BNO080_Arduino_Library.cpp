...
boolean BNO080::begin(TwoWire &wirePort, uint8_t deviceAddress, uint8_t intPin)
{
...
{
	uint8_t bytes_read = _i2cPort->i2c_read_device((char *)shtpHeader, 4);
	if(bytes_read != 4)
		return false;
	...
	cout<<"receivePacket :: "<<" datalength : "<<dataLength<<endl;
	if (dataLength == 0)
	{
...
boolean BNO080::getData(uint16_t bytesRemaining)
{
	...
	while (bytesRemaining > 0)
	{
		...
		uint8_t bytes_read = _i2cPort->i2c_read_device(temp_buffer, numberOfBytesToRead + 4);
		if(bytes_read != numberOfBytesToRead + 4){
			cout<<"getData()::wrong bytes read\n";
			return false;
		}
		for (uint8_t x = 4; x < (numberOfBytesToRead + 4); x++)
		{
			...
			{
				shtpData[dataSpot++] = temp_buffer[x]; //Store data into the shtpData array
			}
		}
...
boolean BNO080::sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
	...
	char packets[dataLength + 4];
	
	packets[0] = packetLength & 0xFF;
	packets[1] = packetLength >> 8;
	packets[2] = channelNumber;
	packets[3] = sequenceNumber[channelNumber]++;
	for (uint8_t i = 0; i < dataLength; i++)
		packets[i+4] = shtpData[i];
	cout<<"Writing "<<packetLength<<" bytes of block data\n";
	int response = _i2cPort->i2c_write_device(packets, packetLength);

	if (response != 0){
		cout<< "Error occured while sending. ERR_CODE::"<<response<<endl;
		return false;
	}
	return true;
}
